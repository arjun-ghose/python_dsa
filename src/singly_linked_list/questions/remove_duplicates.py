...
Your method should not create a new list, but rather modify the existing list in-place, preserving the relative order of
the nodes.
...
2. Without using a Set - This approach will have a time complexity of O(n^2), where n is the number of nodes in the
   linked list. You are not allowed to use any additional data structures for this implementation.
"""
...
class LinkedList:
    ...
    def remove_duplicates(self):
        ...
        while current is not None:
            current_length = len(seen_values)
            seen_values.add(current.value)
            if len(seen_values) > current_length:
                previous = current
                current = current.next
            else:
